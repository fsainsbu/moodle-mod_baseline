 ...
 * Used when editing a baseline template
 */
...
function showHideAdvSearch(checked) {
    ...
    for(i=0;i<divs.length;i++) {
        if(divs[i].id.match('baseline_adv_form')) {
            if(checked) {
...
M.baseline_filepicker = {};


M.baseline_filepicker.callback = function(params) {
 ...
 */
M.baseline_filepicker.init = function(Y, options) {
    options.formcallback = M.baseline_filepicker.callback;
...
M.baseline_urlpicker = {};

M.baseline_urlpicker.init = function(Y, options) {
    options.formcallback = M.baseline_urlpicker.callback;
    if (!M.core_filepicker.instances[options.client_id]) {
        M.core_filepicker.init(Y, options);
    }
    Y.on('click', function(e, client_id) {
...
M.baseline_urlpicker.callback = function (params) {
    document.getElementById('field_url_'+params.client_id).value = params.url;
};

M.baseline_imagepicker = {};

M.baseline_imagepicker.callback = function(params) {
 ...
 * This fucntion is called for each file picker on page.
 */
M.baseline_imagepicker.init = function(Y, options) {
    options.formcallback = M.baseline_imagepicker.callback;
    if (!M.core_filepicker.instances[options.client_id]) {
        M.core_filepicker.init(Y, options);
    }
    Y.on('click', function(e, client_id) {
