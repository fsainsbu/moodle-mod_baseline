...
class baseline_field_picture extends baseline_field_base {
    var $type = 'picture';
    var $previewwidth  = 50;
    ...
    function display_add_field($recordid=0) {
        ...
        if ($recordid) {
            if ($content = $DB->get_record('baseline_content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid))) {
                file_prepare_draft_area($itemid, $this->context->id, 'mod_baseline', 'content', $content->id);
                if (!empty($content->content)) {
                    if ($file = $fs->get_file($this->context->id, 'mod_baseline', 'content', $content->id, '/', $content->content)) {
        ...
        if ($file) {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php/', $this->context->id.'/mod_baseline/content/'.$content->id.'/'.$file->get_filename());
        ...
        $str .= '<label for="field_'.$this->field->id.'_alttext">'.get_string('alttext','baseline') .'</label>&nbsp;<input type="text" name="field_'
        ...
        $module = array('name'=>'baseline_imagepicker', 'fullpath'=>'/mod/baseline/baseline.js', 'requires'=>array('core_filepicker'));
        $PAGE->requires->js_init_call('M.baseline_imagepicker.init', array($fp->options), true, $module);
        return $str;
    }

    // TODO delete this function and instead subclass baseline_field_file - see MDL-16493
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($template == 'listtemplate') {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$this->context->id.'/mod_baseline/content/'.$content->id.'/'.'thumb_'.$content->content);
            // no need to add width/height, because the thumb is resized properly
            $str = '<a href="view.php?d='.$this->field->baselineid.'&amp;rid='.$recordid.'"><img src="'.$src.'" alt="'.s($alt).'" title="'.s($title).'" style="border:0px" /></a>';

        } else {
            $src = file_encode_url($CFG->wwwroot.'/pluginfile.php', '/'.$this->context->id.'/mod_baseline/content/'.$content->id.'/'.$content->content);
    ...
    function update_field() {
        ...
        // Get the old field baseline so that we can check whether the thumbnail dimensions have changed
        $oldfield = $DB->get_record('baseline_fields', array('id'=>$this->field->id));
        ...
        if ($oldfield && ($oldfield->param4 != $this->field->param4 || $oldfield->param5 != $this->field->param5)) {
            ...
            if ($contents = $DB->get_records('baseline_content', array('fieldid'=>$this->field->id))) {
                $fs = get_file_storage();
                if (count($contents) > 20) {
                    echo $OUTPUT->notification(get_string('resizingimages', 'baseline'), 'notifysuccess');
                    echo "\n\n";
                ...
                foreach ($contents as $content) {
                    ...
                    if ($thumbfile = $fs->get_file($this->context->id, 'mod_baseline', 'content', $content->id, '/', 'thumb_'.$content->content)) {
                        $thumbfile->delete();
                    }
                    @set_time_limit(300);
                    // Might be slow!
    ...
    function update_content($recordid, $value, $name) {
        ...
        // Quickly make one now!
            ...
            $id = $DB->insert_record('baseline_content', $content);
            $content = $DB->get_record('baseline_content', array('id'=>$id));
        }
            ...
            case 'file':
                ...
                if (count($files)<2) {
                    ...
                    foreach ($files as $draftfile) {
                        $file_record = array('contextid'=>$this->context->id, 'component'=>'mod_baseline', 'filearea'=>'content', 'itemid'=>$content->id, 'filepath'=>'/');
                        if (!$draftfile->is_directory()) {
                            ...
                            $DB->update_record('baseline_content', $content);
                            $this->update_thumbnail($content, $file);
