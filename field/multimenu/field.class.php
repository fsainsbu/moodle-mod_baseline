...
class baseline_field_multimenu extends baseline_field_base {
    ...
    function display_add_field($recordid=0) {
        ...
        if ($recordid){
            $content = $DB->get_field('baseline_content', 'content', array('fieldid'=>$this->field->id, 'recordid'=>$recordid));
    ...
    function display_search_field($value = '') {
        ...
        if ($used = $DB->get_records_sql($sql, array($this->field->id))) {
            foreach ($used as $baseline) {
                $valuestr = $baseline->content;
                if ($valuestr === '') {
                    continue;
                }
        ...
        $str .= html_writer::checkbox('f_'.$this->field->id.'_allreq', null, $allrequired, get_string('selectedrequired', 'baseline'));
    ...
    function update_content($recordid, $value, $name='') {
        ...
        $content->content  = $this->format_baseline_field_multimenu_content($value);
    ...
    function format_baseline_field_multimenu_content($content) {
        if (!is_array($content)) {
            return NULL;
        }
        $options = explode("\n", $this->field->param1);
