...
*/
        ...
        foreach($this->_datafields as $field) {
            if($field->text_export_supported()) {
                $mform->addElement('advcheckbox', 'field_'.$field->field->id, '<div title="' . s($field->field->name) . '">' . $field->field->param5 . '</div>', ' (' . $field->name() . ')', array('group'=>1));
                //$mform->addElement('advcheckbox', 'field_'.$field->field->id, '<div title="' . s($field->field->name) . '">' . $field->field->param5 . '</div>', ' '  , array('group'=>1));
                //$mform->addElement('advcheckbox', 'field_'.$field->field->id, $field->field->param5  );
         $chked = 0;
         if ($myff= $DB->get_records_select('baseline_field2user', 'userid = '. $this->userid. ' AND field_id = '. $field->field->id.' order by id desc')) {
