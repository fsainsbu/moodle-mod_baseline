...
class PiePlot3DC extends PiePlot3D {
    ...
    function Stroke($img,$aaoption=0) {
        ...
        $radians = $degrees / 2* M_PI;

        if( $this->imidsize > 0.2 && $aaoption !== 2 ) {

            if( $this->ishadowcolor != "") {
                $img->SetColor($this->ishadowcolor ('green','red','orange'));
               // $img->FilledCircle($xc+$this->ishadowdrop,$yc+$this->ishadowdrop,
                // round($radius*$this->imidsize));
			//FilledEllipse($this->img,round($xc),round($yc),round($radius*$this->imidsize),round($radius*$this->imidsize)-200,$this->current_color);
			//imageSmoothArc (&$img->img, round($xc-1.5),round($yc-1.5),round($radius*2*100/$this->imidsize) ,round($radius*2*100/$this->imidsize-39),array(155,155,49,0 ),0, 6.28);
			//imageSmoothArc (&$img->img, round($xc+1.5),round($yc+1.5),round($radius*2*100/$this->imidsize),round($radius*2*100/$this->imidsize-39),array(255,255,255,0 ),0, 6.28);
            }
        
            //$img->SetColor($this->imidcolor);
            $radians = $degrees / 2*M_PI;
            //$img->FilledCircle($xc,$yc,round($radius*$this->imidsize));
	   //imageellipse($img->img, round($xc),round($yc),round($radius*$this->imidsize),round($radius*$this->imidsize-20),$red);
imageSmoothArc (&$img->img, round($xc-0),round($yc-0),round($radius*2*$this->imidsize),round($radius*2*$this->imidsize-39),array(150,150,60,60 ),0, 6.28);
imageSmoothArc (&$img->img, round($xc+0),round($yc+4.5),round($radius*2*$this->imidsize),round($radius*2*$this->imidsize-39),array(255,255,255,0 ),0, 6.28);
		...
		// print to one side if less than 50 % FPS
           if( $this->imidsize > 0.50 ) $this->midtitle->SetPos($xc,$yc,'center','center'); else
	$this->midtitle->SetPos($xc/5,$yc,'center','center');
