...
/// If it's hidden then it doesn't show anything.  :)
    ...
    $PAGE->set_pagelayout('standard');
    $PAGE->navbar->add($strbaselinebases);
 ...
 if ($base) {
	 ...
	 if ((!$rid) && (!$base)) {    //  Norid, is there one already for today and So do you have access?
        ...
        foreach($contents as $content) {
           ...
           redirect($CFG->wwwroot.'/mod/baseline/bedit.php?d='.$baseline->id.'&amp;rid='.$field->id.'&amp;sesskey='.sesskey());
		}
        }
    } else {
...
/// Process incoming baseline for adding/updating records
    ...
    if ($baselinerecord = data_submitted($CFG->wwwroot.'/mod/baseline/bedit.php'.'&amp;base='.$base) and confirm_sesskey()) {
...
/// Upload records section. Only for teachers and the admin.
    ...
    if (has_capability('mod/baseline:manageentries',$context)) {
        if ($import) {
            ...
            echo '<div style="text-align:center">';
            echo '<a href="bedit.php?d='.$baseline->id.'&amp;base='.$base.'&amp;import=1">'.get_string('uploadrecords', 'baseline').'</a>';
            echo '</div>';
        }
    }
