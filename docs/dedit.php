 ...
 if ($base) {
    ...
    if (!$rid) {    //  Norid, is there one already for today and So do you have access?
        ...
        foreach($contents as $content) {
            $field = $DB->get_record('baseline_records','id',$content->id);
...
/// Process incoming baseline for adding/updating records
    ...
    if ($baseline->addtemplate){
        ...
        //$possiblefields = baseline_->get_fields($baseline->id);
        ...
        foreach ($possiblefields as $eachfield){
            ...
            $patterns[]="[[".$field->field->name."]]";
 //need to remove unused fields from template .... e not chosen and not asked today  FPS
      if (($eachfield->param4[date('w')] == '1') && ($myff= $DB->->get_records_select('baseline_field2user', 'userid = '. $USER->id. ' AND field_id = '. $field->field->id,'id DESC')))
{
            $replacements[] = $field->field->description;
            // remove previous $replacements[] =$field->get_bases($rid). $field->display_graph($rid).$field->display_add_field($rid).$field->display_icns($rid);
